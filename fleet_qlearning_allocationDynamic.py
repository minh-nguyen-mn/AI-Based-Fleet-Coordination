...
def qlearning_allocation(vehicles_df, tasks_df, current_time=0):
    ...
    for b in bids:
        ...
        # ------------------------------------------------------------------
        # LONG-HORIZON REWARD: High-Throughput & Energy-Efficient Occupancy
        # ------------------------------------------------------------------
        # 1. Occupancy Reward (Throughput & Idle driver)
        # Strong reward for being busy to beat Greedy's high activity.
        r_busy = 2000.0 * b['et']
        
        # 2. Travel Penalty (Efficiency driver)
        # Increased penalty to ensure energy_per_unit beats the Greedy baseline.
        r_eff = -1200.0 * b['tt']
        
        # 3. Capture & Urgency
        r_capture = 200000.0 + 30000.0 * task['Urgency']
        
        # 4. Strategic Position (Chain potential)
        r_chain = 20000.0 * b['state'][1] # dense_bin
        
        reward = r_busy + r_eff + r_capture + r_chain
        
        # Stability
        next_max_q = 120000.0 if b['state'][1] > 0 else 40000.0
    ...
    for v_idx in idle_v_indices:
        if v_idx not in assigned_v:
            ...
            _agent.update(idle_state, -10000.0) 
