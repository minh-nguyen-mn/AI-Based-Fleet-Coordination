...
class DecentralizedVehicleAgent:
    ...
    Uses an Annealed Boltzmann Temperature to guarantee exploration-exploitation
    balance within a strict 100-step simulation window.
    """
    ...
    def get_local_state(self, vehicle, tasks_df, idle_positions, current_time):
        ...
        # 4. Adjusted Temporal Phase (Centered at midpoint of the 100-step run)
    ...
    def select_action(self, state, current_step):
        """
        Exploration Strategy: Boltzmann (Softmax) with dynamic time-based annealing.
        - Steps 0 to 35: Aggressive random discovery
        - Steps 36 to 65: Gradual convergence transition
        - Steps 66 to 100: Heavy policy exploitation
        """
        initial_temp = 2.0
        min_temp = 0.1
        target_converge_step = 65.0  # The Sweet Spot
        
        # Linear annealing schedule up to the target convergence threshold
        if current_step < target_converge_step:
            temp = initial_temp - ((initial_temp - min_temp) * (current_step / target_converge_step))
        else:
            temp = min_temp

        qs = [self.get_q(state, a) for a in range(4)]
        
        # Numerical stability handling for Softmax
        temp = max(temp, 1e-5)
        max_qs = np.max(qs)
        exp_qs = np.exp((np.array(qs) - max_qs) / temp)
...
def qlearning_allocation(vehicles_df, tasks_df, current_time=0):
    """
    Decentralized Execution Flow passing the time-horizon down into the policy logic.
    """
    ...
    for v_idx in idle_indices:
        ...
        action = agent.select_action(state, current_step=current_time)
